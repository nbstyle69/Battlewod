...
| Builds `1.0.53` livrés depuis `master` `ea04ce3` (merge de #261), même commit iOS/Android. iOS **(51)** et non (50) : `autoIncrement: true` du profil `production` (source de version locale) incrémente `buildNumber` au build — accepté par Nab ; `verify:ipa` 19/19 (Supabase, GIPHY, DSN Sentry, build 51), IPA soumis à TestFlight seul (submission `fc2af340`), rien vers App Review. Android AAB `versionCode` 65 : `verify:aab` 23/23 avec `MAPS_KEY_EXPECTED_SUFFIX=vIwnw` / `MAPS_KEY_FORBIDDEN_SUFFIX=rZMEE` — clé Maps restreinte embarquée, ancienne clé ouverte absente ; AAB déposé par Nab dans la Play Console. `EXPO_PUBLIC_SENTRY_SMOKE=1` était dans l'env EAS : le premier lancement envoie `sentry-smoke v1.0.53 · …`, variable à retirer ensuite | 5 septembre 2026 |
